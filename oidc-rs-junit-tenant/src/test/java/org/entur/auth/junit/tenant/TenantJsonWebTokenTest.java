...
class TenantJsonWebTokenTest {
	...
	void testPartnerTenant(
			@PartnerTenant(clientId = "abc", organisationId = 123L, username = "thomas") String authorization,
	...
	void testTravellerTenant(
			@TravellerTenant(clientId = "abc", organisationId = 123L, customerNumber = "def") String authorization,
			TenantAnnotationTokenFactory factory) {
	...
	void testPersonTenant(
			@PersonTenant(clientId = "abc", organisationId = 123L, socialSecurityNumber = "11223344556") String authorization,
			TenantAnnotationTokenFactory factory) {
	...
	void testAddMyOwnMocking(
			@TravellerTenant(clientId = "abc", organisationId = 123L, customerNumber = "def") String authorization,
			TenantAnnotationTokenFactory factory, WireMockServer mockServer) throws IOException, URISyntaxException {
		String body = "test response";
		mockServer.stubFor(get(urlEqualTo("/test"))
				.willReturn(aResponse().withStatus(200).withHeader("Content-Type", "text/plain").withBody(body)));
