...
class DefaultMdcRequestFilterTest {
	...
	@Test
	void testDefaltMdcForPartner(@PartnerTenant(clientId = "clientId1234", subject = "subject", organisationId = 1234L) String token) throws Exception {
		...
		mockMvc.perform(get("/protected").headers(requestHeaders))
				.andExpect(status().isOk());

		var logsList = listAppender.list;
		boolean foundClientId = logsList.stream().anyMatch(event -> {
			String mdcValue = event.getMDCPropertyMap().get("clientId");
			return "clientId1234".equals(mdcValue);
		});
		boolean foundOrganisationId = logsList.stream().anyMatch(event -> {
