...
public class JwtTokenFactory {
    ...
    private final Map<String, Map<String, KeyPair>> keyPairsByTenantByProvider = new ConcurrentHashMap<>();
    ...
    /**
     * Constructs a {@code TenantTokenFactory} with a collection of providers.
     * Initializes cryptographic key pairs for each provider and tenant.
     *
     * @param provider to add to TenantTokenFactory
     * @param tenants to register for provider
     */
    ...
    public void addTenants(Provider provider, String... tenants) {
        var keyPairsMap = keyPairsByTenantByProvider.computeIfAbsent(provider.getName(), providerName -> {
            providerMap.put(provider.getName(), provider);
           return new HashMap<>();
        });
        Arrays.stream(tenants).forEach(tenant -> keyPairsMap.computeIfAbsent(tenant, key -> keyGen.generateKeyPair()));
    }
    ...
    public String generateJwtToken(@NonNull Provider provider, @NonNull String domain, String subject, String audience, Map<String, Object> claims, int expiresInMinutes) {
        ...
        if(providerMap == null) {
            throw new IllegalArgumentException("Unknown domain " + domain + " for provider " + provider.getName());
        }
        ...
        JwtBuilder jwtBuilder = Jwts.builder().header()
                ...
                .add("typ", "JWT")
                .and()
                .issuer(provider.getIssuerUrl(domain)).expiration(expiresAt);
        ...
        if(claims != null) {
            claims.entrySet().stream()
                    ...
                    .forEach(stringObjectEntry -> jwtBuilder.claim(provider.mapClaimName(stringObjectEntry.getKey()), stringObjectEntry.getValue()));
        }
    ...
    /**
     * Creates a mapping of certificate paths to their corresponding JSON Web Key Set (JWKS) responses.
     *
    ...
    public Map<String, String> createCertificates() {
        ...
        for (Entry<String, Map<String, KeyPair>> keyPairsByTenantByProviderEntry : keyPairsByTenantByProvider
                .entrySet()) {
            for (Entry<String, KeyPair> entry : keyPairsByTenantByProviderEntry.getValue().entrySet()) {
                ...
                String certEndpoint = providerMap.get(keyPairsByTenantByProviderEntry.getKey()).getCertPath(entry.getKey());
